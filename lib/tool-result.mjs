// LIMITATION: A proxy that coordinates multiple traps can present a fabricated,
// internally consistent array view. This module detects single-trap length lies,
// but pure in-process validation cannot establish truth beyond the observed view.
...
const ARRAY_LENGTH_KEYS_MISMATCH = Symbol('array length/keys mismatch');
...
// snapshot + validate length ONCE, then index-based copy where holes become
// undefined (rejected as malformed). A length that mutates mid-copy (lying
// getter/proxy) must not change how many elements we read.
function materialize(arr) {
  const len = arr.length;
  if (!Number.isInteger(len) || len < 0 || len > 1_000_000) {
    throw new Error('malformed array length');
  }
  const numericOwnKeys = new Set(Reflect.ownKeys(arr).filter((key) => {
    if (typeof key !== 'string' || !/^(0|[1-9]\d*)$/.test(key)) return false;
    const index = Number(key);
    return index < 0xFFFFFFFF && String(index) === key;
  }));
  if ([...numericOwnKeys].some(key => Number(key) >= len)) {
    throw ARRAY_LENGTH_KEYS_MISMATCH;
  }
  const out = [];
  for (let i = 0; i < len; i++) {
...
export function normalizeResult(raw) {
  ...
  try {
    ...
    return emptyEnvelope({
      ...
      errors: [diagnostic(error === ARRAY_LENGTH_KEYS_MISMATCH
        ? 'array length disagrees with numeric own keys'
...
export function validateResult(env) {
  try {
    ...
    if (!Array.isArray(findings)) errors.push(diagnostic('findings must be an array'));
      ...
      materializedFindings = materialize(findings);
      materializedFindings.forEach((finding, index) => {
    ...
    if (state === 'success') {
      ...
      if (materializedFindings !== null && materializedFindings.length === 0 && zeroFindings !== true) {
        errors.push(diagnostic('successful zero-findings claim requires zeroFindings: true'));
      }
      if (zeroFindings === true && materializedFindings !== null && materializedFindings.length > 0) {
    ...
    return { ok: false, errors: [diagnostic(error === ARRAY_LENGTH_KEYS_MISMATCH
      ? 'array length disagrees with numeric own keys'
