...
function normalizeProvenance(value) {
  ...
  if (rev !== undefined) provenance.rev = rev;
...
export function normalizeResult(raw) {
  ...
  try {
    ...
    else if (Array.isArray(redactions) && redactions.every(item => typeof item === 'string')) {
...
function validateFinding(finding, index) {
  ...
  if (id !== undefined && (typeof id !== 'string' || id.trim() === '')) {
    return diagnostic(`finding ${index}.id is invalid`);
  }
...
export function validateResult(env) {
  try {
    ...
    if (!Array.isArray(redactions) || !redactions.every(item => typeof item === 'string')) {
