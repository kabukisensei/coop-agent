...
export function normalizeResult(raw) {
  ...
  try {
    ...
    if (rawErrors === undefined) {
      ...
      for (const item of Array.from(rawErrors)) {
    ...
    if (Array.isArray(rawFindings)) {
      Array.from(rawFindings).forEach((value, index) => {
    ...
    if (redactions === undefined) result.redactions = [];
      ...
      const materializedRedactions = Array.from(redactions);
...
export function validateResult(env) {
  try {
    ...
    else Array.from(findings).forEach((finding, index) => {
      const findingError = validateFinding(finding, index);
      if (findingError) errors.push(findingError);
    });
    if (!Array.isArray(resultErrors) || !Array.from(resultErrors).every(item => typeof item === 'string' && item.startsWith('ToolResult:'))) {
      errors.push(diagnostic('errors must be an array of ToolResult strings'));
    }
    if (!Array.isArray(redactions) || !Array.from(redactions).every(item => typeof item === 'string')) {
