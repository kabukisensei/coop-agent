"""Contain a managed Windows shell command and its descendants.

The child starts suspended and joins the job before any candidate code runs.
Cancellation terminates the job and waits for every process to exit. Normal
completion preserves intentional background processes. Supervisor failure closes
the job's sole handle and terminates contained descendants.
"""
import ctypes
...
def run(argv):
    if sys.platform != "win32" or len(argv) < 2 or not all(os.path.isabs(p) for p in argv[:2]):
        raise ValueError("Windows job requires an absolute executable")
    cancel_file, argv = argv[0], argv[1:]
    ...
    started = False
    try:
        # Hold the exact agent process object: PID reuse must not extend a job
        # after the agent has crashed or been force-closed.
        owner = check(k.OpenProcess(0x100000, False, os.getppid()))
        ...
        started = True
        while True:
            wait = k.WaitForSingleObject(info.hProcess, 50)
            if wait not in (0, 258):
                raise ctypes.WinError(ctypes.get_last_error())
            cancelled = os.path.exists(cancel_file) or k.WaitForSingleObject(owner, 0) == 0
            if wait == 0 or cancelled:
                break
        result = w.DWORD()
        check(k.GetExitCodeProcess(info.hProcess, ctypes.byref(result)))
        if cancelled:
            check(k.TerminateJobObject(job, 130))
            ...
            while True:
                ...
                if time.monotonic() >= deadline:
                    raise RuntimeError("Windows shell descendants did not stop")
                time.sleep(0.025)
        # A completed command may intentionally leave a background process.
        ...
        # Cancellation/failure keeps KILL_ON_JOB_CLOSE armed.
        k.CloseHandle(job)
        if owner:
            k.CloseHandle(owner)
        if info.hThread:
...
if __name__ == "__main__":
    try:
        ...
        print(f"Windows shell job failed: {error}", file=sys.stderr)
        sys.exit(1)
