...
# Add fallback tool bins without overriding an explicit, test-stubbed, or managed PATH.
if [ "${COOP_DESKTOP_MANAGED_RUNTIME:-0}" != "1" ]; then
[ -d "$HOME/.local/bin" ] && case ":$PATH:" in *":$HOME/.local/bin:"*) : ;; *) PATH="$PATH:$HOME/.local/bin" ;; esac
[ -n "${COOP_TEST_STUB_PATH:-}" ] || [ ! -d "/opt/homebrew/bin" ] || case ":$PATH:" in *":/opt/homebrew/bin:"*) : ;; *) PATH="$PATH:/opt/homebrew/bin" ;; esac
[ -n "${COOP_TEST_STUB_PATH:-}" ] || [ ! -d "/usr/local/bin" ] || case ":$PATH:" in *":/usr/local/bin:"*) : ;; *) PATH="$PATH:/usr/local/bin" ;; esac
