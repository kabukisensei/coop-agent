...
/** Strict PR-identity validation: regex candidate must also parse as an
 *  https URL with a dotted host and a provider-shaped PR path. Output like
 *  "https://]/pull/7" must NOT satisfy the publication gate. */
...
// Single-repo (PR) mode only. Parse the mode key from NON-COMMENT lines with
// exact value comparison: a `# mode: self` comment or a `mode: self-destruct`
// value must NOT satisfy the safety gate.
function assertPrMode(config) {
  ...
  const modes = readFileSync(marker, "utf8").split(/\r?\n/)
    .map((line) => line.trim())
    .filter((line) => line && !line.startsWith("#"))
    .map((line) => /^mode\s*:\s*(\S+)\s*$/.exec(line))
    .filter(Boolean)
    .map((m) => m[1]);
  if (modes.length !== 1 || modes[0] !== "self") return diag("unsafe-mode", "Checkout is not exactly single-repo/PR mode; direct-push contribution against an approved branch is refused.");
  return ok(true);
}
