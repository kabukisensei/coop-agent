...
// lines may declare the mode — a nested `metadata:\n  mode: self` must not
// promote a checkout whose actual root mode is `team`. Lines with trailing
// inline comments fail the strict match and therefore fail CLOSED (absence of
// a clean root `mode: self` is refusal, never authorization).
function assertPrMode(config) {
  ...
  const modes = readFileSync(marker, "utf8").split(/\r?\n/)
    ...
    .map((line) => /^mode\s*:\s*(\S+)\s*$/.exec(line.trim()))
    .filter(Boolean)
    .map((m) => m[1]);
  if (modes.length !== 1 || modes[0] !== "self") return diag("unsafe-mode", "Checkout root mode is not exactly 'self'; direct-push contribution against an approved branch is refused.");
  return ok(true);
}
