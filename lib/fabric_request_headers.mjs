...
export function windowsAzureCliCommand(cmd, cli, resource = FABRIC_RESOURCE) {
  ...
  const commandLine = `"${invocation} ${args.join(" ")} & set "COOP_AZURE_RC=!ERRORLEVEL!" & echo ${WINDOWS_STDOUT_SENTINEL.toString("ascii")}!COOP_AZURE_RC! & echo ${WINDOWS_STDERR_SENTINEL.toString("ascii")} 1>&2 & set /p "COOP_AZURE_RELEASE=" & exit /b !COOP_AZURE_RC!"`;
  return {
    command: cmd,
...
function acquireToken(commandSpec, env) {
  return new Promise((resolve, reject) => {
    ...
    const terminateTree = (done) => {
      ...
      const spec = windowsTaskkillCommand(env.SystemRoot, child.pid);
    ...
    const fail = (kind = "command") => {
      ...
      clearTimeout(timer);
      removeSignalHandler();
      terminateTree(() => reject(new TokenError(kind)));
    };
    const authError = () => {
      let message = "";
    ...
    child.on("close", (code) => {
      ...
      settled = true;
      removeSignalHandler();
      resolve(completion?.stdout || stdout);
    });
  });
}
