...
export function normalizeSources(config, options = {}) {
  ...
  for (let index = 0; index < sourceDefinitions.length; index++) {
    ...
    identities.add(find(claimIdentityKey(raw, index)));
  ...
  // Circuit breaker: with v2 definitions gated off, their explicit ids remain
  // RESERVED. An emitted record reusing such an id is excluded fail-closed — a
  // gated v2 definition's id can never be claimed by another repository or
  // scope through the default path.
  const reservedGatedIds = new Set();
  if (!allowV2) {
    for (const raw of v2Definitions) {
      if (isObject(raw) && isNonEmptyString(raw.id)) reservedGatedIds.add(raw.id.trim());
    }
  }
  ...
  const add = (entry) => {
    // Duplicate detection uses completed union-find alias classes, not literal
    // locators: two records whose identities share a class are one source.
    const conflicts = entries.filter((existing) =>
      [...entry.roots].some((root) => existing.roots.has(root))
    ...
    winner.roots = new Set([...combinedIdentities].map(find));
  ...
  const prepare = (entry) => {
    ...
    entry.roots = new Set([...entry.identities].map(find));
  ...
  const isConflictBlocked = (entry) => {
    if (![...entry.roots].some((root) => [...conflictBlockedIdentities].some((blocked) => find(blocked) === root))) return false;
  ...
  for (const raw of legacyDefinitions) {
    ...
    if (entry && reservedGatedIds.has(entry.record.id)) {
      errors.push(diagnostic("id-reserved-by-disabled-v2", "source id is reserved by a disabled v2 definition", entry.record.id));
      continue;
    }
    if (entry && explicitClaims.has(entry.record.id)) {
      ...
      continue;
    } else if (entry && ![...entry.roots].some((root) => [...blockedIdentities].some((blocked) => find(blocked) === root))) add(entry);
