...
export function normalizeSources(config, options = {}) {
  ...
  for (const raw of [...v2Definitions, ...legacyDefinitions]) {
    ...
    for (const identity of definitionIdentities(raw)) {
      ...
      if (isObject(raw) && (raw.enabled === false || raw.disabled === true)) v2DisabledIdentities.add(identity);
      if (isObject(raw) && ((raw.enabled !== undefined && typeof raw.enabled !== "boolean") || (raw.disabled !== undefined && typeof raw.disabled !== "boolean"))) v2DisabledIdentities.add(identity);
    }
  }
  ...
  for (const entry of entries) {
    ...
    if (!entry.explicit && [...entry.identities].some((identity) => v2DisabledIdentities.has(identity))) {
      sources.push({ ...entry.record, enabled: false });
      continue;
    }
