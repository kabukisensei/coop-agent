...
function slugFor(identity) {
  ...
  // Keep lossy punctuation from making unrelated bindings share a generated id.
  return /[^a-z0-9/.-]/i.test(identity) ? `${slug}-${hashFor(identity)}` : slug;
}
...
function normalizeRecord(raw, legacy, warnings, errors, blockedIdentities) {
  ...
  return { record: result, identities, explicit: !legacy, generatedId: !label };
}
...
export function normalizeSources(config) {
  ...
  const add = (entry) => {
    const conflicts = entries.filter((existing) =>
      ...
      || (existing.explicit && entry.explicit && existing.record.id === entry.record.id)
    );
    if (!conflicts.length) {
      entries.push(entry);
      return;
  ...
  const explicitIds = new Set(entries.filter((entry) => entry.explicit).map((entry) => entry.record.id));
