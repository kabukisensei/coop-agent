...
function slugFor(identity) {
  const slug = identity.replace(/^local:/, "local-").toLowerCase()
    .replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "source";
  // A qualified repository (the canonical owner/repository form) retains its
  // readable slug.  An unqualified locator is always salted from itself, so
  // adding another source can never change the first source's generated id.
  // Other lossy punctuation is salted as well.
  return (!identity.includes("/") || /[^a-z0-9/.-]/i.test(identity))
    ? `${slug}-${hashFor(identity)}`
    : slug;
}
...
function normalizeRecord(raw, legacy, warnings, errors, blockedIdentities) {
  ...
  return { record: result, identities, explicit: !legacy, explicitId: Boolean(label), generatedId: !label };
}
...
export function normalizeSources(config) {
  ...
  for (const raw of Array.isArray(root.repos) ? root.repos : []) {
    ...
    if (entry && ![...entry.identities].some((identity) => blockedIdentities.has(identity))) add(entry);
  ...
  // Resolve lossy generated-slug collisions only after all binding conflicts
  // have been reconciled.  In particular, do not let an explicit id consume a
  // generated v2 id, or let an early generated record discard a later one.
  const generatedById = new Map();
  const explicitIds = new Set(entries.filter((entry) => entry.explicitId).map((entry) => entry.record.id));
  for (const entry of entries) {
    if (!entry.generatedId) continue;
    const group = generatedById.get(entry.record.id) || [];
    group.push(entry);
    generatedById.set(entry.record.id, group);
  }
  for (const group of generatedById.values()) {
    if (group.length < 2 && !explicitIds.has(group[0].record.id)) continue;
    for (const entry of group) {
      const identity = [...entry.identities].sort()[0];
      entry.record.id = `${entry.record.id}-${hashFor(identity)}`;
    }
  }

  // A generated id can also collide with an explicit id.  The explicit
  // definition remains canonical; the generated id is made deterministic
  // from its own locator, never from source ordering or its neighbours.
  for (const entry of entries) {
    if (entry.generatedId && explicitIds.has(entry.record.id)) {
      const identity = [...entry.identities].sort()[0];
      entry.record.id = `${entry.record.id}-${hashFor(identity)}`;
    }
  }
