...
export function normalizeSources(config) {
  ...
  for (const raw of sourceDefinitions) {
    const entry = prepare(normalizeRecord(raw, false, warnings, errors, blockedIdentities));
  ...
  for (const raw of legacyDefinitions) {
    const entry = prepare(normalizeRecord(raw, true, warnings, errors, blockedIdentities));
