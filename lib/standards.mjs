...
const MODEL_RE = /\b(semantic model|tabular model|star schema|fact table|dimension table|tmdl)\b|\.bim\b/i;
const DOC_RE = /\b(documentation|document|readme|glossary|metadata description)\b/i;
const FABRIC_RE = /\b(microsoft fabric|fabric workspace|fabric notebook|fabric data pipeline|lakehouse|warehouse|deployment pipeline|medallion architecture)\b/i;
...
function bundledResolution(domain, options) {
  ...
  try {
    ...
    const revision = typeof info.revision === "string" && info.revision.trim()
      ? info.revision.trim()
      : (typeof report.version === "string" && report.version.trim() ? `reviewer-${report.version.trim()}` : null);
...
export function verifyReviewerProvenance(resolution, report) {
  ...
  try {
    ...
    if (typeof returned.path !== "string" || resolve(returned.path) !== resolve(resolution.path)) return { ok: false, error: "reviewer standards path mismatch" };
    if (realpathSync(returned.path) !== realpathSync(resolution.path)) return { ok: false, error: "reviewer standards path mismatch" };
    if (returned.sha256 !== resolution.sha256) return { ok: false, error: "reviewer standards hash mismatch" };
    if (returned.revision !== resolution.revision) return { ok: false, error: "reviewer standards revision mismatch" };
    return { ok: true };
