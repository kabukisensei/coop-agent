...
import MANAGED_STANDARDS_REGISTRY from "../config/standards-registry.json" with { type: "json" };
...
export function standardsRegistry(options = {}) {
  ...
  if (options.registryPath && !fixture) throw new Error("alternate standards registry is allowed only for an explicit fixture");
  const registry = fixture ? JSON.parse(readFileSync(options.registryPath, "utf8")) : structuredClone(MANAGED_STANDARDS_REGISTRY);
