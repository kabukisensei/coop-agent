...
export function resolveStandard(domain, options = {}) {
  ...
  const generation = options.taskPin || activeCanonicalGeneration(options);
  if (generation?.ok) {
...
export function pinStandardsTask(domains, options = {}) {
  ...
  const taskPin = activeCanonicalGeneration(options);
  const pinnedOptions = { ...options, refresh: false, taskPin };
  // Resolve every supported domain now. The immutable snapshots outlive
  // bounded generation retention, so retrieval or a reviewer invoked late in
  // a long task cannot fall through to a newer pointer.
  const pinnedDomains = domains.length ? DOMAINS : [];
  const byDomain = new Map(pinnedDomains.map((domain) => [domain, resolveStandard(domain, pinnedOptions)]));
  const resolutions = domains.map((domain) => byDomain.get(domain));
  const resolvePinned = (domain) => byDomain.get(domain) || resolveStandard(domain, pinnedOptions);
  return Object.freeze({ taskPin, refresh, resolutions: Object.freeze(resolutions), resolve: resolvePinned });
}
...
export function buildStandardsContext(prompt, options = {}) {
  ...
  return Object.freeze({ domains, records: Object.freeze(records), patterns: Object.freeze(patterns), refresh, taskPin: pinned.taskPin, resolve: pinned.resolve });
}
...
export function validateReviewerReport(domain, report) {
  ...
  if (!exactKeys(report.standards, ["path", "sha256"]) || typeof report.standards.path !== "string" || !fingerprint(report.standards.sha256)) return { ok: false, error: "reviewer standards provenance schema is invalid" };
  return { ok: true };
}
...
export function promoteReviewRun(outdir, entries, options = {}) {
  ...
  for (const entry of entries) {
    ...
    if (resolution.domain !== entry.domain) return { ok: false, error: `${entry.domain} report rejected: resolution domain mismatch` };
  ...
  const stage = join(generations, `.stage-${generationId}`);
  ...
  try {
    for (const entry of verified) {
      writeDurable(join(stage, `coop-${entry.domain}-review.json`), readFileSync(entry.reportPath));
    ...
    const files = Object.fromEntries(verified.flatMap((entry) => [
      [`${entry.domain}_report`, { file: `coop-${entry.domain}-review.json`, sha256: sha256(join(stage, `coop-${entry.domain}-review.json`)) }],
    ...
    const metadata = { schema_version: 1, generation_id: generationId, domains: verified.map((x) => x.domain).sort(), files };
    ...
    const active = resolveAcceptedReviewRun(root);
    if (!active.ok) throw new Error(active.error);
    return { ok: true, generation_id: generationId, bindings: Object.fromEntries(verified.map((x) => [x.domain, x.binding])), reports: active.reports, binding_paths: active.bindings };
  } catch (e) { return { ok: false, error: `review run promotion failed: ${e.message}` }; }
  finally { rmSync(stage, { recursive: true, force: true }); }
}

export function resolveAcceptedReviewRun(outdir) {
  try {
    const root = resolve(outdir), pointerPath = join(root, "active-review-generation.json");
    ...
    if (!plainObject(pointer) || pointer.schema_version !== 1 || !/^review-[A-Za-z0-9.-]+$/.test(pointer.generation_id || "") || !/^[0-9a-f]{64}$/.test(pointer.metadata_sha256 || "")) throw new Error("active review generation pointer is invalid");
    const generation = join(root, "accepted-generations", pointer.generation_id);
    ...
    const metadataPath = join(generation, "generation.json"), metadata = safeJson(metadataPath);
    if (!regularUnsymbolic(metadataPath) || sha256(metadataPath) !== pointer.metadata_sha256 ||
        !plainObject(metadata) || metadata.schema_version !== 1 || metadata.generation_id !== pointer.generation_id ||
        metadata.domains?.slice().sort().join("|") !== "dax|sql" || !plainObject(metadata.files) ||
        Object.keys(metadata).sort().join("|") !== "domains|files|generation_id|schema_version" ||
    ...
    const reports = {}, bindings = {};
    for (const domain of metadata.domains) for (const [kind, target] of [["report", reports], ["binding", bindings]]) {
      const entry = metadata.files?.[`${domain}_${kind}`], path = entry?.file ? join(generation, entry.file) : "";
      if (!plainObject(entry) || Object.keys(entry).sort().join("|") !== "file|sha256" || !isSafeRelativePath(entry.file) ||
          !/^[0-9a-f]{64}$/.test(entry.sha256 || "") || !regularUnsymbolic(path) || !inside(path, generation) || sha256(path) !== entry.sha256) throw new Error(`active review ${domain} ${kind} is invalid`);
      target[domain] = realpathSync(path);
    }
    return Object.freeze({ ok: true, generation_id: pointer.generation_id, generation: realpathSync(generation), reports: Object.freeze(reports), bindings: Object.freeze(bindings) });
...
export function sourceStatus(options = {}) {
  ...
  const domains = Object.fromEntries(DOMAINS.map((d) => {
    const resolution = resolveStandard(d, { ...options, taskPin: statusPin, refresh: false });
...
function fsyncDirectory(path) {
  try { const fd = openSync(path, "r"); try { fsyncSync(fd); } finally { closeSync(fd); } } catch { /* Windows may not fsync directories */ }
}
...
function verifyCanonicalCheckout(checkout, options, expectedRevision = null) {
  ...
  return errors.length ? { ok: false, error: errors.join("; ") } : { ok: true, revision };
}
...
export function activeCanonicalGeneration(options = {}) {
  try {
    ...
    if (!plainObject(pointer) || pointer.schema_version !== 1 || !/^[0-9a-f]{40}-[A-Za-z0-9]+$/.test(pointer.generation_id || "") || !/^[0-9a-f]{40}$/.test(pointer.revision || "") || !/^[0-9a-f]{64}$/.test(pointer.metadata_sha256 || "")) throw new Error("active canonical generation pointer is invalid");
    ...
    if (!plainObject(metadata) || metadata.schema_version !== 1 || metadata.generation_id !== pointer.generation_id || metadata.revision !== pointer.revision || metadata.repository !== c.repository || metadata.branch !== c.authoritative_branch || metadata.anchor !== c.initial_verified_commit || metadata.index_sha256 !== sha256(indexPath)) throw new Error("canonical generation metadata/pointer disagreement");
    const checkoutResult = verifyCanonicalCheckout(checkout, options, pointer.revision);
    if (!checkoutResult.ok) throw new Error(checkoutResult.error);
    ...
    return Object.freeze({ ok: true, generation_id: pointer.generation_id, revision: pointer.revision, generation: realpathSync(generation), checkout: realpathSync(checkout), index: realpathSync(indexPath) });
  } catch (e) { return { ok: false, error: e.message }; }
}
...
export function refreshCanonical(options = {}) {
  ...
  if (active.ok && (previous.revision !== active.revision || previous.generation_id !== active.generation_id)) {
    previous = { ...previous, ok: true, state: "fresh", degraded: false, revision: active.revision, generation_id: active.generation_id, last_successful_check_ms: now, last_successful_sync_ms: previous.last_successful_sync_ms || now, repository: c.repository, branch: c.authoritative_branch, last_attempt_ms: now, reconciled: true };
    atomicJson(statePath, previous);
  }
  const age = now - Number(previous.last_successful_check_ms || 0);
  if (!options.force && previous.ok === true && previous.degraded !== true && active.ok && previous.revision === active.revision && previous.generation_id === active.generation_id && previous.last_successful_check_ms && age < c.freshness_seconds * 1000) return { ok: true, skipped: true, state: "fresh", revision: active.revision, generation_id: active.generation_id, changed: false };
  const runOptions = { ...options, timeoutMs: options.timeoutMs || c.timeout_seconds * 1000 };
  ...
  try {
    ...
    const metadata = { schema_version: 1, generation_id: generationId, revision, repository: c.repository, branch: c.authoritative_branch, anchor: c.initial_verified_commit, index_sha256: sha256(indexPath), domains: index.domains };
    writeDurable(join(stage, "generation.json"), JSON.stringify(metadata, null, 2) + "\n");
    ...
    pruneGenerations(storage.generations, generationId, options.retainGenerations || 3);
  } catch (e) {
    active = activeCanonicalGeneration(options);
...
function pruneGenerations(root, activeId, retain) {
  try {
    ...
    const keep = new Set([activeId, ...rows.slice(0, Math.max(1, retain)).map((x) => x.name)]);
    for (const row of rows) if (!keep.has(row.name)) rmSync(join(root, row.name), { recursive: true, force: true });
