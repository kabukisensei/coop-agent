...
export function standardsRegistry(options = {}) {
  ...
  if ((options.registryPath || options.remote) && !fixture) throw new Error("alternate standards registry or remote is allowed only for an explicit fixture");
  const registry = fixture && options.registryPath ? JSON.parse(readFileSync(options.registryPath, "utf8")) : structuredClone(MANAGED_STANDARDS_REGISTRY);
