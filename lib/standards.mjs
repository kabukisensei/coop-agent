...
const FABRIC_RE = /\b(microsoft fabric|fabric workspace|workspace|lakehouse|warehouse|notebook|data pipeline|deployment pipeline|medallion architecture)\b/i;
...
function bundledResolution(domain, options) {
  ...
  try {
    ...
    return Object.freeze({
      domain, authority_class: "formal_standard", state: "bundled_fallback", path: realpathSync(path),
      revision: report.version ? `reviewer-${report.version}` : "reviewer-bundled", sha256: info.sha256,
      source: `coop-${domain}-review:bundled`, source_root: dirname(realpathSync(path)),
    });
  } catch { return null; }
...
export function resolveStandard(domain, options = {}) {
  ...
  if (["sql", "dax"].includes(domain)) {
    ...
    if (bundled) try { return snapshot(bundled); } catch { /* retain fail-soft legacy state below */ }
    return Object.freeze({ domain, authority_class: "formal_standard", state: "bundled_fallback", path: null, revision: null, sha256: null, source: `coop-${domain}-review:bundled` });
  }
...
export function verifyReviewerProvenance(resolution, report) {
  if (!resolution?.path) return { ok: true };
  try {
    ...
    const returned = report && typeof report === "object" ? report.standards : null;
    if (returned) {
      if (typeof returned.path !== "string" || realpathSync(returned.path) !== realpathSync(resolution.path)) return { ok: false, error: "reviewer standards path mismatch" };
      if (returned.sha256 !== resolution.sha256) return { ok: false, error: "reviewer standards hash mismatch" };
      if (returned.source && returned.source !== resolution.source) return { ok: false, error: "reviewer standards source mismatch" };
      if (returned.revision && returned.revision !== resolution.revision) return { ok: false, error: "reviewer standards revision mismatch" };
    }
    return { ok: true };
