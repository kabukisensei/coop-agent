...
export class WorkspaceLeaseManager {
  ...
  readOwnerRecord(ownerPath) {
    try {
      ...
      try {
        ...
        return { ok: false, exists: true, code: "invalid-json", error, owner: null };
      }
      if (validOwner(value)) {
        return { ok: true, exists: true, owner: value };
      }
      return { ok: false, exists: true, code: "invalid-schema", owner: null };
    } catch (error) {
      if (error?.code === "ENOENT") {
        return { ok: true, exists: false, owner: null };
      }
      return { ok: false, exists: true, code: "unreadable", error, owner: null };
    }
  }

  readOwner(ownerPath) {
    return this.readOwnerRecord(ownerPath).owner;
  }

  readOverrides(lockDir) {
    ...
    try {
      ...
      for (const entry of readdirSync(directory, { withFileTypes: true })) {
        ...
        if (!owner || owner.accessMode !== "override") return null;
        owners.push(owner);
      }
      return owners;
  ...
  readActiveOverrides(lockDir) {
    const overrideDir = join(lockDir, "overrides");
    try {
      const files = readdirSync(overrideDir);
      const jsonFiles = files.filter((f) => f.endsWith(".json"));
      const overrides = [];
      for (const f of jsonFiles) {
        const p = join(overrideDir, f);
        const record = this.readOwnerRecord(p);
        if (!record.ok || !record.owner) {
          return { ok: false, code: record.code || "unreadable-override", file: f, overrides: [], liveOverrides: [] };
        }
        overrides.push({ path: p, file: f, owner: record.owner });
      }
      const liveOverrides = overrides.filter((o) => !this.isOwnerRecoverable(o.owner));
      return { ok: true, overrides, liveOverrides };
    } catch (error) {
      if (error?.code === "ENOENT") {
        return { ok: true, overrides: [], liveOverrides: [] };
      }
      return { ok: false, code: "unreadable-override-dir", error, overrides: [], liveOverrides: [] };
    }
  }
  ...
  inspect(cwd) {
    ...
    const overrideRes = this.readActiveOverrides(paths.lockDir);
    if (!overrideRes.ok) {
      return { ok: false, state: "invalid", code: "workspace-lease-invalid", message: "Workspace override lease metadata could not be read or is invalid." };
    }
    const { overrides, liveOverrides } = overrideRes;

    const ownerRecord = this.readOwnerRecord(paths.ownerPath);
    if (!ownerRecord.ok) {
      return { ok: false, state: "invalid", code: "workspace-lease-invalid", message: "Workspace lease metadata is unreadable or invalid." };
    }

    let owner = ownerRecord.owner;
    ...
    if ((!owner || this.isOwnerRecoverable(owner)) && liveOverrides.length > 0) {
      const promoted = liveOverrides[0].owner;
      try {
        this.writeJsonAtomic(paths.ownerPath, promoted);
        owner = promoted;
      } catch {
        owner = promoted;
      }
    ...
    if (!owner) {
      ...
      owner = overrides[0].owner;
    }

    // An owner that explicitly released its lease (releasedAt) is recoverable
    // once no override writers remain; otherwise staleness rules apply.
    const explicitlyReleased = typeof owner.releasedAt === "string" && Number.isFinite(Date.parse(owner.releasedAt));
    const baseRecoverable = this.isOwnerRecoverable(owner) || explicitlyReleased;
    const recoverable = baseRecoverable && liveOverrides.length === 0;
    ...
    if (this.active) {
      ...
      if (!released) {
        return {
          ok: false,
          state: "invalid",
          code: "workspace-lease-invalid",
          message: "Could not release existing workspace lease.",
        };
      }
    }
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (!held.recoverable || !held.owner) {
        return {
          ...
          state: held.state,
          code: held.code,
          message: held.state === "invalid"
            ? (held.message || "Workspace lease metadata is missing or invalid.")
            : `This checkout is already writable in ${held.owner?.clientInterface || "another Coop client"}.`,
  ...
  heartbeat() {
    ...
    const currentRecord = this.readOwnerRecord(path);
    if (!currentRecord.ok) {
    ...
    try {
      ...
      if (this.active.overridePath) {
        const baseOwnerRecord = this.readOwnerRecord(this.active.ownerPath);
        if (baseOwnerRecord.ok) {
          const baseOwner = baseOwnerRecord.owner;
          const shouldPromote = (!baseOwner && !baseOwnerRecord.exists)
            || (baseOwner && (baseOwner.leaseId === current.leaseId || this.isOwnerRecoverable(baseOwner)));
          if (shouldPromote) {
            try { this.writeJsonAtomic(this.active.ownerPath, current); } catch { /* ignore */ }
          }
        }
      }
      this.active.owner = current;
      return true;
    } catch { return false; }
  }
  ...
  release() {
    ...
    const current = currentRecord.owner;
    if (!current || current.leaseId !== active.owner.leaseId) {
      this.stopHeartbeat();
      this.active = null;
      return false;
    }

    const overrideRes = this.readActiveOverrides(active.lockDir);
    if (!overrideRes.ok) {
      return false;
    }
    const { overrides, liveOverrides } = overrideRes;
    ...
    if (active.overridePath) {
      const otherLiveOverrides = liveOverrides.filter((o) => o.owner.leaseId !== active.owner.leaseId);
      const baseOwnerRecord = this.readOwnerRecord(active.ownerPath);
      if (!baseOwnerRecord.ok) {
        return false;
      }
      const baseOwner = baseOwnerRecord.owner;
      const isBaseOwner = (!baseOwner && !baseOwnerRecord.exists)
        || (baseOwner && (baseOwner.leaseId === active.owner.leaseId || this.isOwnerRecoverable(baseOwner)));

      if (isBaseOwner && otherLiveOverrides.length > 0) {
        const next = otherLiveOverrides[0];
        try {
          this.writeJsonAtomic(active.ownerPath, next.owner);
        } catch {
          return false;
        }
      }
      ...
      try {
        unlinkSync(active.overridePath);
      } catch {
        return false;
      }

      if (otherLiveOverrides.length === 0) {
        const overrideDir = dirname(active.overridePath);
        try { rmSync(overrideDir, { recursive: true, force: true }); } catch { /* ignore */ }
        if (isBaseOwner) {
          try { rmSync(active.lockDir, { recursive: true, force: true }); } catch { /* ignore */ }
        }
      }
    ...
    if (overrides.length > 0) {
      const overrideDir = join(active.lockDir, "overrides");
      try { rmSync(overrideDir, { recursive: true, force: true }); } catch { /* ignore */ }
    }
    ...
    const remainingOverrides = this.readOverrides(active.lockDir);
    if (remainingOverrides === null || remainingOverrides.length > 0) {
