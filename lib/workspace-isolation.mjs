...
async function runGit(cwd, args, { maxBuffer = 1024 * 1024 } = {}) {
  try {
    ...
    const result = await execFileAsync("git", [...platformArgs, "-C", cwd, ...args], {
      encoding: "utf8",
      windowsHide: true,
      maxBuffer,
      shell: false,
...
export class WorkspaceLeaseManager {
  ...
  inspect(cwd) {
    ...
    const overrides = this.readOverrides(paths.lockDir);
  ...
  release() {
    ...
    const overrides = this.readOverrides(active.lockDir);
...
export async function createManagedWorktree({ cwd, agentDir, ownerId, approved = false, createId = () => randomUUID() } = {}) {
  ...
  const roots = managedRoots(agentDir);
  // IDs and their records are already unique across this agent's repositories.
  // Omitting the redundant 64-character bucket on Windows keeps Git's worktree
  // root below its fixed GIT_DIR buffer even when core.longpaths is enabled.
  // Existing records retain their exact paths and remain valid for cleanup.
  const checkoutParent = process.platform === "win32"
    ? join(roots.checkouts, id)
    : join(roots.checkouts, workspaceLeaseKey(repository.gitCommonDir || repository.repositoryRoot), id);
