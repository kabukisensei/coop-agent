...
import { resolveCoopBuildIdentity } from "./coop-build-identity.mjs";
import { sourceStatus } from "./standards.mjs";

// This checkout's own root (never the user's project cwd) — the source of the
// trustworthy build revision for support bundles.
const COOP_ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "..");

// Platform-aware profile root: COOP_DIR wins; else the real user home
// (os.homedir() works on Windows where HOME is typically unset — never the
// working directory).
const COOP_DIR = process.env.COOP_DIR || join(homedir() || process.env.HOME || ".", ".coop");
...
const standardsStatus = sourceStatus({ cwd: process.cwd() });
const standardsUsable = Object.values(standardsStatus.domains).every((r) =>
  Boolean(r && r.path && r.revision && r.sha256)
);
probes.push({
  component: "standards",
  status: standardsUsable ? "ok" : "degraded",
  detail: `canonical remote ${standardsStatus.canonical_remote}; ${standardsStatus.sources.map((s) => `${s.id}=${s.state}@${s.revision || "none"}`).join(", ")}; domains ${Object.entries(standardsStatus.domains).map(([name, r]) => `${name}=${r.state}`).join(", ")}`,
  checkedAt: now(),
});
if (incidentMode) {
...
const buildIdentity = resolveCoopBuildIdentity(COOP_ROOT);
const versions = {
  node: process.version,
  platform: `${process.platform}/${process.arch}`,
  coopBuild: buildIdentity.ok ? fingerprintBuild(buildIdentity.value).value : "build-unavailable",
};

// --- events (sanitize-then-persist; retention includes the new record) ------
mkdirSync(SUPPORT_DIR, { recursive: true });
const sanitizeLine = (l) => {
  try {
    const s = sanitizeSupportEvent(JSON.parse(l));
    return JSON.stringify(s.ok ? s.value : { sanitize_error: s.errors });
  } catch {
    // content-free marker: a malformed line must never persist its raw text
    // (it could carry a credential we cannot parse to redact)
    return JSON.stringify({ unparseable_event: true, length: l.length });
  }
};
if (existsSync(EVENTS)) {
  const lines = readFileSync(EVENTS, "utf8").split("\n").filter((l) => l.trim());
  // every retained event is rewritten THROUGH the contract — the raw log
  // never keeps credentials (F1)
  writeFileSync(EVENTS, lines.slice(-MAX_EVENT_LOG_LINES).map(sanitizeLine).join("\n") + "\n");
}
let rawEvents = [];
if (existsSync(EVENTS)) {
  rawEvents = readFileSync(EVENTS, "utf8").split("\n").filter((l) => l.trim())
    .map((l) => { try { return JSON.parse(l); } catch { return { unparseable: l.slice(0, 200) }; } });
}
...
const bundle = {
  ...
  versions,
  standards: sanitizeSupportEvent(standardsStatus).value,
...
// then re-trim so retention INCLUDES the new record (F2)
try {
  const record = sanitizeSupportEvent({ event: incidentMode ? "support-incident" : "support-run", run: bundle.run, at: now(), components: mStatusSummary(bundle) });
  appendFileSync(EVENTS, JSON.stringify(record.ok ? record.value : { sanitize_error: record.errors }) + "\n");
  const all = readFileSync(EVENTS, "utf8").trim().split("\n").filter(Boolean);
  if (all.length > MAX_EVENT_LOG_LINES) writeFileSync(EVENTS, all.slice(-MAX_EVENT_LOG_LINES).join("\n") + "\n");
