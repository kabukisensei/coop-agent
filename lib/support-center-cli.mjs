...
probes.push({
  component: "standards",
  status: standardsStatus.sources.find((s) => s.id === "cooptimize-formal-standards")?.state === "available" ? "ok" : "degraded",
  detail: `canonical remote ${standardsStatus.canonical_remote}; ${standardsStatus.sources.map((s) => `${s.id}=${s.state}@${s.revision || "none"}`).join(", ")}`,
  checkedAt: now(),
});
if (incidentMode) {
