...
def choose_tenant(tenants: list[dict[str, str]], *, login_result: bool = False) -> str:
    ...
    if len(tenants) == 1:
        if login_result:
            sys.stderr.write(
                f"✓ Signed in. Detected client tenant {tenant_label(tenants[0])}.\n"
            )
...
def sign_in_and_detect_tenant() -> str:
    ...
    if read_confirm("Try again with a device code?", default=True):
        ...
        if ok and tenants:
            return choose_tenant(tenants, login_result=True)
    sys.stderr.write(
        "Azure is not connected. Enter the tenant manually or leave it blank and configure it later.\n"
    )
    return ""
...
def build_project_yml(answers: dict) -> str:
    ...
    repositories = answers.get("repositories") or [
        {
            ...
            "description": answers["repo_description"],
            "local_path": answers["repo_local_path"],
            "remote_name": answers["repo_remote_name"],
    ...
    estate_mode = (
        "connected"
        if "mixed" in roles or {"sql", "powerbi"}.issubset(roles)
        else ("partial" if repositories else "discovery")
    )
    lines.append("estate:")
    lines.append(f"  mode: {quote(estate_mode)}")
    lines.append("  local_source_coverage:")
    lines.append(
        f"    sql: {quote('available' if 'sql' in roles or 'mixed' in roles else ('unknown' if 'generic' in roles else 'not_available_yet'))}"
    )
    lines.append(
    ...
    for repo in repositories:
        ...
        lines.append(
            f"    default_branch: {quote(repo.get('default_branch', answers['default_branch']))}"
        )
    ...
    if answers.get("use_fabric"):
        ...
        lines.append("  default_sql_endpoint:")
        lines.append('    item_type: ""')
        lines.append('    item_name: ""')
        lines.append('    item_id: ""')
        lines.append("    sqlEndpointProperties:")
        lines.append('      id: ""')
        lines.append(
        ...
        lines.append('  default_workspace_name: ""')
        lines.append('  default_workspace_id: ""')
        lines.append("  # Semantic-model workspace for each deployment environment.")
        lines.append("  environment_names:")
        lines.append('    dev: ""')
        lines.append('    test: ""')
        lines.append('    prod: ""')
    ...
    lines.append(
        "  # Pinned catalog refreshed by `coop sync`; legacy source/load_dir are ignored."
    )
...
def run_wizard(target_dir: Path) -> dict:
    ...
    repo_description = ask_prefilled(
    ...
    while read_confirm("Add another repository?", default=False):
        ...
        extra_description = ask_prefilled(
            "Additional repository description", "Project source and docs"
        )
        extra_branch = ask_prefilled("Additional repository default branch", "main")
        extra_role = read_choice(
            "Additional repository role",
            ["sql", "powerbi", "mixed", "generic"],
            default="generic",
        )
        repositories.append(
            {
                ...
                "remote_name": "origin",
                "default_branch": extra_branch,
            }
        )

    use_fabric = read_confirm(
        "Is this a Microsoft Fabric / Power BI engagement?", default=False
    )
    tenant_id = ""
    if use_fabric:
    ...
    if use_tabular_editor:
        te_path = ask_prefilled(
            "Tabular Editor CLI command or executable path", discover_te_path()
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--template",
        action="store_true",
        help="Print the full legacy template instead of running the wizard.",
    )
