...
function normalizeSource(source) {
  ...
  const root = realpathSync(resolve(source.root));
  if (!statSync(root).isDirectory()) throw new TypeError("Knowledge source root must be a directory.");
  let gitRoot;
  try {
    gitRoot = realpathSync(execFileSync("git", ["-C", root, "rev-parse", "--show-toplevel"], {
      encoding: "utf8",
  ...
  if (!(root === gitRoot || root.startsWith(`${gitRoot}${sep}`))) throw new TypeError("Knowledge source is outside its Git repository root.");
  return { ...source, root };
}
