...
/** Safe string for diagnostics: never throws on cyclic or exotic values. */
const describe = (x) => {
...
const makeSupportId = (kind) => (date, hex) => {
  const errors = [];
  if (!DATE_RE.test(date)) errors.push(`${kind}-id: date must be YYYY-MM-DD (got ${describe(date)})`);
  if (!HEX8_RE.test(hex)) errors.push(`${kind}-id: hex must be 8 lowercase hex chars (got ${describe(hex)})`);
  if (errors.length) return diag(errors);
...
export const fingerprintBuild = (build) => {
  if (!isObject(build)) return diag(["build-fingerprint: input must be an object"]);
  if (!isNonEmptyString(build.version)) return diag(["build-fingerprint: version is required"]);
  if (!isNonEmptyString(build.commit)) return diag(["build-fingerprint: commit is required"]);
