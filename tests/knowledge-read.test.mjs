...
await t("keeps valid files when malformed records do not hide listed files", async () => {
  const result = await readSourceDocuments(source, {
    async listFiles() { return ["good.md"]; },
    async readFiles() { return [
      { path: "good.md", text: "# Good" },
...
await t("reports incomplete reads even when another record is missing its path", async () => {
  const result = await readSourceDocuments(source, {
    async listFiles() { return ["good.md", "missing.md"]; },
    async readFiles() { return [
      { path: "good.md", text: "# Good" },
      { text: "missing path" },
    ]; },
  ...
  assert.deepEqual(result.documents, []);
  assert.match(result.errors[0], /incomplete-read-files/);
  assert.match(result.errors[0], /missing\.md/);
});
...
await t("does not close a fence with mixed marker characters", async () => {
  const { documents } = await readSourceDocuments(source, {
    async listFiles() { return ["heading.md"]; },
    async readFiles() { return [{ path: "heading.md", text: "```md\n# Fenced\n```~\n# Still fenced\n```\n# Real" }]; },
  });
