...
await test("native main health acknowledgement follows renderer readiness and runtime shutdown", async () => {
  ...
  calls.length = 0;
  const failedWrite = vm.runInContext(`(async () => {${source.slice(begin, end)}})()`, ctx);
  const rejectedWrite = assert.rejects(failedWrite, /pipe closed/);
  await new Promise(resolve => setImmediate(resolve));
  assert.deepEqual(calls, ["rpc", "stop", "ack"]);
  flush(new Error("pipe closed"));
  await rejectedWrite;
  assert.deepEqual(calls, ["rpc", "stop", "ack"], "a failed health write must reject before the success exit");
