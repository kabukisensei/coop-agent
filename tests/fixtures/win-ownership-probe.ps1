...
if (-not $IsWindows) { Write-Host '  - win ownership probe: skipped (non-Windows)'; exit 0 }
...
if k32 is None:
    ...
    print("PROBE| jobconfig.create_handle=%s getlasterror=%d" % (job, e1))
    created = k32._win_job_create()
    print("PROBE| jobconfig.product_create=%s" % ("ok" if created else "NONE"))
    if created:
...
'@ | Set-Content -Path $src -Encoding Ascii
  ...
  if ($LASTEXITCODE -eq 0) { & $exe } else { $script:probeFailed = $true; Write-Host 'PROBE| crt.compile_failed' }
