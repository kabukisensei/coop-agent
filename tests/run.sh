...
echo "→ Desktop preview lifecycle and native-boundary tests"
node "$ROOT/tests/desktop-preview-shell.test.mjs"
echo "→ managed Desktop runtime bundle tests"
...
node "$ROOT/tests/windows-update-process.test.mjs"
...
node "$ROOT/tests/update-service.test.mjs"
node "$ROOT/tests/update-trust.test.mjs"

echo "→ Desktop release evidence and parity gate tests"
node "$ROOT/tests/desktop-release-gate.test.mjs"
echo "→ runtime capability negotiation tests"
node "$ROOT/tests/runtime-capabilities.test.mjs"
echo "→ structured execution and runtime event contract tests"
node "$ROOT/tests/runtime-domain.test.mjs"
echo "→ declarative service extension contract tests"
node "$ROOT/tests/service-extensions.test.mjs"
echo "→ resumable workflow extension contract tests"
node "$ROOT/tests/workflow-extensions.test.mjs"
echo "→ shared Doctor service contract tests"
node "$ROOT/tests/doctor-service.test.mjs"
echo "→ shared authentication provider contract tests"
node "$ROOT/tests/auth-service.test.mjs"
echo "→ shared project configuration proposal/write contract tests"
node "$ROOT/tests/project-config-service.test.mjs"
echo "→ read-only environment discovery contract tests"
node "$ROOT/tests/environment-discovery.test.mjs"
echo "→ progressive project setup state contract tests"
node "$ROOT/tests/project-setup-service.test.mjs"
echo "→ Pi RPC adapter and runtime client tests"
node "$ROOT/tests/rpc-adapter.test.mjs"
node "$ROOT/tests/usage-windows.test.mjs"
node "$ROOT/tests/rpc-questionnaire.test.mjs"
node "$ROOT/tests/questionnaire-compat.test.mjs"
echo "→ session tree projection tests"
node "$ROOT/tests/session-tree-model.test.mjs"
echo "→ commands, images, and live queue projection tests"
node "$ROOT/tests/interaction-model.test.mjs"
echo "→ workspace onboarding and Health projection tests"
node "$ROOT/tests/workspace-health-model.test.mjs"
echo "→ allowlisted capability-view and generic findings projection tests"
node "$ROOT/tests/capability-view-registry.test.mjs"
node "$ROOT/tests/findings-model.test.mjs"
node "$ROOT/tests/findings-golden.test.mjs"
node "$ROOT/tests/lineage-model.test.mjs"
node "$ROOT/tests/impact-analysis.test.mjs"
echo "→ shared user profile owner/service tests"
node "$ROOT/tests/profile-service.test.mjs"
echo "→ existing-branch navigation adapter tests"
COOP_TEST_DIST="$TMP" node "$ROOT/tests/tree-navigation.test.mjs"
echo "→ cross-client workspace isolation tests"
node "$ROOT/tests/workspace-isolation.test.mjs"
echo "→ governed knowledge scope and lifecycle policy tests"
node "$ROOT/tests/knowledge-policy.test.mjs"
echo "→ disposable Git-backed knowledge index tests"
node "$ROOT/tests/knowledge-index.test.mjs"
echo "→ governed knowledge source-change and presentation tests"
node "$ROOT/tests/knowledge-service.test.mjs"
node "$ROOT/tests/knowledge-model.test.mjs"
echo "→ workspace-first Mission Control projection tests"
node "$ROOT/tests/mission-control-model.test.mjs"
echo "→ shared three-theme design-system tests"
node "$ROOT/tests/theme-system.test.mjs"
echo "→ clipboard and attachment portability tests"
node "$ROOT/tests/content-portability.test.mjs"
node "$ROOT/tests/clipboard-interoperability.test.mjs"
node "$ROOT/tests/session-export.test.mjs"
echo "→ cross-client writable session lease tests"
COOP_TEST_DIST="$TMP" node "$ROOT/tests/session-lease.test.mjs"
echo "→ safe terminal handoff contract tests"
COOP_TEST_DIST="$TMP" node "$ROOT/tests/terminal-handoff.test.mjs"
echo "→ supported Coop Runtime entry-point tests"
node "$ROOT/tests/runtime-entrypoint.test.mjs"
