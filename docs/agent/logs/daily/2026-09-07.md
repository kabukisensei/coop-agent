...
## TeamAI shared-knowledge integration assessment

- User asked whether https://github.com/Tencent/teamai-cli fits Coop shared
  knowledge and how to integrate terminal/Desktop. Read upstream README, usage
  guide, agent registry, recall and contribution implementation; observed upstream
  main revision 0ec7b77b3663b9f72bb8b0b4532e722be6c1c159.
- Strong conceptual fit: Git-backed shared resources, recall, session-learning
  suggestions and reviewed resource distribution. No built-in Coop/Pi entry was
  present in the reviewed agent registry; custom skill paths do not establish
  lifecycle-hook or native integration compatibility.
- Coop already implements knowledge policy, approved-record index, preview/apply
  service and shared Hub. Terminal parity remains partial; source onboarding and
  sync remain unfinished. Recommended architecture is one Coop Core knowledge
  service with optional TeamAI integration, shared by terminal tools/commands and
  Desktop UI. Preserve private memory, client-project records and sanitized team
  records as distinct scopes; retain human lifecycle approval and source citations.
- Upstream standalone contribute directly pushes learnings and indexes them before
  push succeeds; self-repo mode uses PRs but also indexes pending contributions.
  Neither behavior should make unapproved records authoritative in Coop. Use a
  reviewed draft/import boundary. Keep SQL/DAX/model lineage in coop-data-doc.
- Proposed first implementation: one private team repo, controlled local sync,
  approved-only scoped recall with citations in both clients, then reviewed capture.
  No TeamAI install/init, provider authentication, settings injection or external
  publication performed. This is an assessment, not verified runtime compatibility.
- Sources: upstream docs/usage-guide.md, src/known-agents.ts, src/recall.ts,
  src/contribute.ts; local docs/knowledge-policy.md, lib/knowledge-{policy,index,
  service}.mjs, web/server.mjs and config/desktop-parity.json.
...
- The machine-local confidential-sweep commit hook flagged the unchanged dummy
  OPENAI_API_KEY fixture on a modified test line. Compared HEAD and staged values:
  they are identical and explicitly fixture-only; no credential was introduced.
  The hook reported no other finding and is the sole pre-commit hook. Using its
  documented verified-false-positive bypass for this commit only; hook unchanged.
