...
| `warehouse-mcp-live-acceptance` | With an authorized test identity and approved Warehouse/Lakehouse target, `coop doctor` validates the item target and discovers `executeSQL`, `execute_query`, or a documented server-prefixed spelling through `tools/list`. The observation must carry the schema-validated `warehouse_live` object: `auth_state=authenticated`, `target_validation=validated`, `target_scope=item`, the exact discovered compatible tool, `provenance=live`, and `mock=false`. Generic prose, mock/fixture evidence, global scope, or config-only `registered`, `auth_required`, `unavailable`, `tool_missing`, and `target_invalid` are not passes. Do not run SQL or retain tokens, target names, SQL, arguments, or rows in evidence. |
