...
7. **MCP is read-only by default; Warehouse SQL is approval-gated.** Fabric, Power BI, and Learn MCP are for `list` / `read` / `inspect`; create/update/delete/deploy/publish require approval. `fabric-sqlendpoint` is separate from Fabric MCP; every SQL call requires approval, and DDL/DML/destructive SQL is classified before row reads.
