...
7. **MCP is read-only by default; Warehouse SQL is approval-gated.** Fabric, Power BI, and Learn MCP are for `list` / `read` / `inspect`; mutations require approval. `fabric-sqlendpoint` is separate: each call needs approval or a matching session grant; unsafe or ambiguous SQL stays separately gated.
