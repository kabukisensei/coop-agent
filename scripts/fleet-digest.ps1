...
$py = Get-CoopPython
if (-not $py) { [Console]::Error.WriteLine('fleet-digest: python3 (or python) not found on PATH (a Windows Store python stub does not count)'); exit 127 }

& $py (Join-Path $ScriptDir 'fleet-digest.py') @args
exit $LASTEXITCODE
