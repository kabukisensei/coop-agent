#!/usr/bin/env bash
#
# Fetch official Microsoft agent skills into the configured load_dir(s).
#
# Supports two source blocks in .coop/project.yml:
#   - microsoft_skills      (github.com/microsoft/skills)
#   - fabric_skills         (github.com/microsoft/skills-for-fabric)
#
# Only allow-listed, non-conflicting skills are copied. Fetched skills are
# gitignored (pulled on demand, never vendored into this repo) and are
# SUBORDINATE to Cooptimize skills at load time.
#
# Usage: scripts/fetch-microsoft-skills.sh
#
set -uo pipefail

COOP_ROOT="$(cd -P "$(dirname "${BASH_SOURCE[0]}")/.." >/dev/null 2>&1 && pwd)"
export COOP_ROOT
# shellcheck source=../lib/common.sh
. "$COOP_ROOT/lib/common.sh"

have git || coop_die "git is required to fetch Microsoft skills."

proj="$(coop_find_project_yml)"
[ -n "$proj" ] || coop_die "No .coop/project.yml found."

# Build the set of our own skill names (folder + frontmatter) for conflict checks.
own=" "
for d in "$COOP_ROOT"/skills/*/; do
  name="$(basename "$d")"; [ "$name" = "_microsoft" ] || [ "$name" = "_microsoft_fabric" ] && continue
  [ -f "${d}SKILL.md" ] || continue
  own="$own$name "
  fm="$(coop_skill_name "${d}SKILL.md")"; [ -n "$fm" ] && own="$own$fm "
done

fetch_source() {
  local key="$1"
  local cache_dir="$2"

  local src load_dir allow
  src="$(coop_yaml_get "$proj" "$key.source" "")"
  load_dir="$(coop_yaml_get "$proj" "$key.load_dir" "")"
  allow="$(coop_yaml_list "$proj" "$key.allow")"

  case "$src" in ""|TODO*) return 0 ;; esac
  [ -n "$load_dir" ] || load_dir="skills/$key"

  if [ -z "$allow" ]; then
    coop_info "$key.allow[] is empty — skipping."
    return 0
  fi

  local cache dest
  cache="$COOP_ROOT/$cache_dir"
  dest="$COOP_ROOT/$load_dir"

  coop_head "Fetching Microsoft skills from $src"
  if [ -d "$cache/.git" ]; then
    coop_info "Updating cache…"; git -C "$cache" -c http.lowSpeedLimit=1000 -c http.lowSpeedTime=15 pull --ff-only --depth 1 >/dev/null 2>&1 || coop_warn "cache pull failed (using existing)"
  else
    mkdir -p "$(dirname "$cache")"
    coop_info "Cloning (shallow)…"; git clone --depth 1 -c http.lowSpeedLimit=1000 -c http.lowSpeedTime=15 "$src" "$cache" >/dev/null 2>&1 || coop_die "clone failed: $src"
  fi

  mkdir -p "$dest"

  local count=0
  # Feed the loop with a here-doc (not a pipe) so `count` survives in this shell.
  while IFS= read -r skill; do
    [ -z "$skill" ] && continue
    case "$skill" in TODO*) continue ;; esac
    # Reject anything but a safe skill-folder charset (blocks globs / path traversal).
    case "$skill" in .|..|-*|*[!a-zA-Z0-9._-]*) coop_warn "skip '$skill' — invalid skill name"; continue ;; esac
    case "$own" in *" $skill "*) coop_warn "skip '$skill' — conflicts with a Cooptimize skill (subordinate rule)"; continue ;; esac
    # Microsoft skills are stored under skills/, plugins/, or deeper plugin folders.
    local found=""
    while IFS= read -r hit; do found="$(dirname "$hit")"; break; done <<EOF
$(find "$cache" -type f -path "*/$skill/SKILL.md" 2>/dev/null)
EOF
    if [ -z "$found" ]; then coop_warn "not found in source: $skill"; continue; fi
    local fm
    fm="$(coop_skill_name "$found/SKILL.md")"
    case "$own" in *" $fm "*) coop_warn "skip '$skill' — frontmatter name '$fm' conflicts with a Cooptimize skill"; continue ;; esac
    rm -rf "${dest:?}/$skill"
    cp -R "$found" "$dest/$skill"
    coop_ok "fetched $skill"
    count=$((count+1))
  done <<EOF
$allow
EOF

  coop_ok "Done — fetched $count $key skill(s) into $dest (gitignored; subordinate to Cooptimize skills)."
}

fetch_source "microsoft_skills" ".cache/microsoft-skills"
fetch_source "fabric_skills" ".cache/microsoft-skills-for-fabric"
