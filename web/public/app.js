...
function handle(evt) {
  ...
  switch (evt.type) {
    ...
    case "compaction_end":
      statusPhase = "";
      if (!dot.classList.contains("busy")) statusText.textContent = idleStatus();
      refreshCtx();
      break;
    case "__message": {
...
async function switchChat(sid, { force = false } = {}) {
  ...
  renderTabs();
  renderQueue();
  if (mode === "poll") { setCwd(c.cwd); void refreshState(); return; } // the poll tick resets since=0, replays, then opens Desktop Mission Control
  const my = ++switchSeq;
  switching = true;
  pendingLive = [];
  resetTranscript();
